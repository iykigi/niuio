        ...
        <div class="card !p-0 lg:col-span-1">
            ...
            <ul class="max-h-96 overflow-y-auto">
                @forelse ($tables as $table)
                    <li>
                        <button wire:click="selectTable('{{ $table }}')" class="flex w-full items-center gap-2 px-4 py-2 text-left text-sm {{ $selectedTable === $table ? 'bg-harbor-50 text-harbor-700 dark:bg-harbor-500/10 dark:text-harbor-300' : 'text-slate-600 dark:text-slate-300' }}">
                            ...
                            @forelse ($rows as $row)
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap px-3 py-2 text-right">
                                        <button wire:click='openEditRow(@js($row))' class="btn-ghost !p-1.5"><x-heroicon-o-pencil class="h-4 w-4" /></button>
                                        <button wire:click='deleteRow(@js(array_intersect_key($row, array_flip(array_column(array_filter($columns, fn($c) => $c["primary_key"]), "name")))))' wire:confirm="Delete this row?" class="btn-ghost !p-1.5 text-rose-600"><x-heroicon-o-trash class="h-4 w-4" /></button>
                                    </td>
                                </tr>
                            @empty
            ...
            @if ($tab === 'import-export')
                <div class="card space-y-6">
                    ...
                    <div>
                        ...
                        <button wire:click="importSql" class="btn-primary mt-3">Import</button>
                    </div>
                </div>
            @endif
